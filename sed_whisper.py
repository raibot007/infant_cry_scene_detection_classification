...
#######################################################################################
# UPDATED: load_data now loads Whisper features instead of HuBERT
#######################################################################################

def load_data(_feat_folder, _mono, _fold=None):
    feat_file_fold = os.path.join(_feat_folder, 'whisper_{}_fold{}.npz'.format('mon' if _mono else 'bin', _fold))
...
#######################################################################################
# UPDATED: model renamed for Whisper embeddings (but same logic)
#######################################################################################

def get_whisper_model(data_in, data_out, _cnn_nb_filt, _cnn_pool_size, _rnn_nb, _fc_nb):
    print("Input shape:", data_in.shape)

    # Input for Whisper embeddings: (time_frames, embedding_dim, channels)
...
feat_folder = './feat_whisper/'   # <--- UPDATED: folder name changed
...
# Whisper embeddings correspond roughly to 20ms/frame → 50 frames per second
frames_1_sec = 50
...
for fold in [1, 2, 3, 4]:
    ...
    # Load Whisper embeddings and labels
    ...
    model = get_whisper_model(X, Y, cnn_nb_filt, cnn_pool_size, rnn_nb, fc_nb)
