...
def extract_log_mel_spectrogram(_y, _sr, _nfft, _nb_mel):
    # Compute log-mel spectrogram
    log_mel_spec = librosa.feature.melspectrogram(y=_y, sr=_sr, n_fft=_nfft, hop_length=int(_nfft/2), n_mels=_nb_mel, power=1)
...
for audio_filename in os.listdir(audio_folder):
    ...
    if is_mono:
        log_mel_spec = extract_log_mel_spectrogram(y, sr, nfft, nb_mel_bands).T
    else:
        for ch in range(y.shape[0]):
            log_mel_spec_ch = extract_log_mel_spectrogram(y[ch, :], sr, nfft, nb_mel_bands).T
            if log_mel_spec is None:
                log_mel_spec = log_mel_spec_ch
            else:
                log_mel_spec = np.concatenate((log_mel_spec, log_mel_spec_ch), 1)

    label = np.zeros((log_mel_spec.shape[0], len(__class_labels)))
    ...
    np.savez(tmp_feat_file, log_mel_spec, label)
