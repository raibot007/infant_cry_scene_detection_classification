...
wav2vec2_model = torchaudio.pipelines.WAV2VEC2_BASE.get_model().to(device)
wav2vec2_model.eval()

def extract_wav2vec2(_y, _sr):
    """
    Extract pretrained Wav2Vec2 embeddings from the audio signal.
    ...
    embeddings : numpy.ndarray
        Wav2Vec2 features of shape (n_time_frames, n_features)
    """
    ...
    with torch.inference_mode():
        embeddings, _ = wav2vec2_model(waveform)
...
feat_folder = './feat_wav2vec2/'  # Updated feature folder
utils.create_folder(feat_folder)

sr = 16000  # Wav2Vec2 expects 16 kHz
hop_len_wav2vec2 = 320  # ≈20ms hop (same as HuBERT for alignment)
...
for audio_filename in os.listdir(audio_folder):
    ...
    print(f'Extracting Wav2Vec2 features and label for: {audio_file}')

    y, sr = load_audio(audio_file, mono=is_mono, fs=sr)
    embeddings = None

    if is_mono:
        embeddings = extract_wav2vec2(y, sr)
    else:
        for ch in range(y.shape[0]):
            emb_ch = extract_wav2vec2(y[ch, :], sr)
            if embeddings is None:
                embeddings = emb_ch
            else:
                embeddings = np.concatenate((embeddings, emb_ch), axis=1)

    # Create label matrix aligned to Wav2Vec2 frames
    label = np.zeros((embeddings.shape[0], len(__class_labels)))
    tmp_data = np.array(desc_dict[audio_filename])
    frame_start = np.floor(tmp_data[:, 0] * sr / hop_len_wav2vec2).astype(int)
    frame_end = np.ceil(tmp_data[:, 1] * sr / hop_len_wav2vec2).astype(int)
...
for fold in folds_list:
    ...
    normalized_feat_file = os.path.join(feat_folder, 'wav2vec2_{}_fold{}.npz'.format('mon' if is_mono else 'bin', fold))
