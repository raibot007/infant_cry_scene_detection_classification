...
def extract_cqcc(y, sr=8000, B=24, hop_length=128):
    """
...
# Output
feat_folder = './feat_wst_cqt/'  # Feature folder for WST outputs
utils.create_folder(feat_folder)

# User set parameters
B = 24  # Number of CQCC coefficients
...
for audio_filename in os.listdir(audio_folder):
    ...
    if is_mono:
        ...
        cqt = extract_cqcc(y, sr, B=B, hop_length=hop_len_wst)
    else:
        for ch in range(y.shape[0]):
            cqt_ch = extract_cqcc(y[ch, :], sr, B=B, hop_length=hop_len_wst)
            wst_ch = extract_wst(y[ch, :], sr, J=J, Q=Q)
            wst = np.concatenate((wst, wst_ch), axis=1) if wst is not None else wst_ch
            cqt = np.concatenate((cqt, cqt_ch), axis=1) if cqt is not None else cqt_ch

    min_frames = min(wst.shape[0], cqt.shape[0])
    wst = wst[:min_frames, :]
    cqt = cqt[:min_frames, :]
    combined_features = np.concatenate((wst, cqt), axis=1)

    # Create the label matrix
    label = np.zeros((combined_features.shape[0], len(__class_labels)))
    ...
    np.savez(tmp_feat_file, combined_features, label)
