...
def extract_whisper(_y, _sr):
    ...
    """
    waveform = torch.tensor(_y, dtype=torch.float32).unsqueeze(0)
    ...
    # Get encoder input features
    input_features = whisper_processor(waveform.squeeze(0).cpu().numpy(), sampling_rate=16000, return_tensors="pt").input_features.to(device)
...
feat_folder = './feat_whisper/'  # Folder for Whisper outputs
utils.create_folder(feat_folder)

sr = 16000  # Whisper expects 16 kHz
hop_len_whisper = 320  # ~20ms per frame, for alignment
...
for audio_filename in os.listdir(audio_folder):
    ...
    print(f'Extracting Whisper features and label for: {audio_file}')

    y, sr = load_audio(audio_file, mono=is_mono, fs=sr)
    embeddings = None

    if is_mono:
        embeddings = extract_whisper(y, sr)
    else:
        for ch in range(y.shape[0]):
            emb_ch = extract_whisper(y[ch, :], sr)
            if embeddings is None:
                embeddings = emb_ch
            else:
                embeddings = np.concatenate((embeddings, emb_ch), axis=1)

    # Create label matrix aligned to Whisper frames
    label = np.zeros((embeddings.shape[0], len(__class_labels)))
    tmp_data = np.array(desc_dict[audio_filename])
    frame_start = np.floor(tmp_data[:, 0] * sr / hop_len_whisper).astype(int)
    frame_end = np.ceil(tmp_data[:, 1] * sr / hop_len_whisper).astype(int)
...
for fold in folds_list:
    ...
    normalized_feat_file = os.path.join(feat_folder, 'whisper_{}_fold{}.npz'.format('mon' if is_mono else 'bin', fold))
