...
# ---------------- Wav2Vec2-Conformer Setup ----------------
device = 'cuda' if torch.cuda.is_available() else 'cpu'

# Load pretrained Conformer model safely (no .bin vulnerability)
conformer_model = Wav2Vec2ConformerModel.from_pretrained(
    "facebook/wav2vec2-conformer-rel-pos-large",
    use_safetensors=True
).to(device)
conformer_model.eval()

def extract_conformer(_y, _sr):
    """
    Extract pretrained Wav2Vec2-Conformer embeddings from an audio signal.
    ...
    embeddings : numpy.ndarray
        Conformer features of shape (n_time_frames, n_features)
    """
    waveform = torch.tensor(_y, dtype=torch.float32).unsqueeze(0).to(device)

    # Resample to 16 kHz if needed
    if _sr != 16000:
        resampler = T.Resample(_sr, 16000).to(device)
        waveform = resampler(waveform)

    with torch.inference_mode():
        outputs = conformer_model(waveform)
        # outputs.last_hidden_state: [1, n_frames, n_features]
...
def load_audio(filename, mono=True, fs=44100):
    ...
    if file_extension == '.wav':
        ...
        if fs != sample_rate:
            audio_data = librosa.resample(audio_data, orig_sr=sample_rate, target_sr=fs)
            sample_rate = fs
...
feat_folder = './feat_conformer/'  # Feature folder for Conformer outputs
utils.create_folder(feat_folder)

sr = 16000  # Conformer expects 16 kHz
hop_len_conformer = 320  # approx 20ms per frame
...
for audio_filename in os.listdir(audio_folder):
    ...
    print(f'Extracting Conformer features and label for: {audio_file}')

    y, sr = load_audio(audio_file, mono=is_mono, fs=sr)
    embeddings = None

    if is_mono:
        embeddings = extract_conformer(y, sr)
    else:
        for ch in range(y.shape[0]):
            emb_ch = extract_conformer(y[ch, :], sr)
            if embeddings is None:
                embeddings = emb_ch
            else:
                embeddings = np.concatenate((embeddings, emb_ch), axis=1)

    # Create label matrix aligned to Conformer frames
    label = np.zeros((embeddings.shape[0], len(__class_labels)))
    tmp_data = np.array(desc_dict[audio_filename])
    frame_start = np.floor(tmp_data[:, 0] * sr / hop_len_conformer).astype(int)
    frame_end = np.ceil(tmp_data[:, 1] * sr / hop_len_conformer).astype(int)
...
for fold in folds_list:
    ...
    normalized_feat_file = os.path.join(feat_folder, 'conformer_{}_fold{}.npz'.format('mon' if is_mono else 'bin', fold))
