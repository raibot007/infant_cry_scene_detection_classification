...
#######################################################################################
# UPDATED: load_data now loads HuBERT features instead of WST
#######################################################################################

def load_data(_feat_folder, _mono, _fold=None):
    feat_file_fold = os.path.join(_feat_folder, 'hubert_{}_fold{}.npz'.format('mon' if _mono else 'bin', _fold))
...
#######################################################################################
# UPDATED: model renamed for HuBERT embeddings (but same logic)
#######################################################################################

def get_hubert_model(data_in, data_out, _cnn_nb_filt, _cnn_pool_size, _rnn_nb, _fc_nb):
    print("Input shape:", data_in.shape)

    # Input for HuBERT embeddings: (time_frames, embedding_dim, channels)
    ...
    for _i, _cnt in enumerate(_cnn_pool_size):
        ...
        # Apply MaxPooling only on the feature axis (not time)
...
feat_folder = './feat_hubert/'   # <--- UPDATED: folder name changed
...
# HuBERT embeddings correspond roughly to 20ms/frame → 50 frames per second
frames_1_sec = 50
...
for fold in [1, 2, 3, 4]:
    ...
    # Load HuBERT embeddings and labels
    ...
    # Build model
    model = get_hubert_model(X, Y, cnn_nb_filt, cnn_pool_size, rnn_nb, fc_nb)
