...
def load_data(_feat_folder, _mono, _fold=None):
    feat_file_fold = os.path.join(_feat_folder, 'wst_{}_fold{}.npz'.format('mon' if _mono else 'bin', _fold))
    dmp = np.load(feat_file_fold)
    _X_train, _Y_train, _X_test, _Y_test = dmp['arr_0'],  dmp['arr_1'],  dmp['arr_2'],  dmp['arr_3']
    return _X_train, _Y_train, _X_test, _Y_test


def get_wst_model(data_in, data_out, _cnn_nb_filt, _cnn_pool_size, _rnn_nb, _fc_nb):
    print("Input shape:", data_in.shape)

    # Input for WST: (time_frames, scattering_coefficients, channels)
    spec_start = Input(shape=(data_in.shape[-3], data_in.shape[-2], data_in.shape[-1]))  # Input shape is (time_frames, scattering_coefficients, channels)
    spec_x = spec_start
    ...
    for _i, _cnt in enumerate(_cnn_pool_size):
        ...
        # Apply MaxPooling only on the frequency axis (scattering_coefficients), not on the time axis
        spec_x = MaxPooling2D(pool_size=(1, _cnn_pool_size[_i]))(spec_x)  # Keep time dimension intact
        spec_x = Dropout(dropout_rate)(spec_x)

    # Permuting the dimensions to align them for RNN input
    spec_x = Permute((2, 1, 3))(spec_x)  # Move frequency to time axis for RNN processing
    print("Shape after Permute:", spec_x.shape)

    # Reshaping for RNN
    spec_x = Reshape((data_in.shape[-2], -1))(spec_x)  # Reshape to (time_frames, features)
...
seq_len = 128       # Frame sequence length. Input to the CRNN.
...
hop_len_wst = 2 ** J
frames_1_sec = int(sr / hop_len_wst)  # Hop length based on WST
...
cnn_pool_size = [4, 2]   # Maxpooling across frequency. Length of cnn_pool_size =  number of CNN layers
rnn_nb = [16]           # Number of RNN nodes.  Length of rnn_nb =  number of RNN layers
fc_nb = [16]                # Number of FC nodes.  Length of fc_nb =  number of FC layers
...
for fold in [1, 2, 3, 4]:
    ...
    # Load model
    model = get_wst_model(X, Y, cnn_nb_filt, cnn_pool_size, rnn_nb, fc_nb)

    # Training
    best_epoch, pat_cnt, best_er, f1_for_best_er, best_conf_mat = 0, 0, 99999, None, None
    tr_loss, val_loss, f1_overall_1sec_list, er_overall_1sec_list, acc_overall_1sec_list, prec_overall_1sec_list, rec_overall_1sec_list = [0] * nb_epoch, [0] * nb_epoch, [0] * nb_epoch, [0] * nb_epoch, [0] * nb_epoch, [0] * nb_epoch, [0] * nb_epoch
