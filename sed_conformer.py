...
#######################################################################################
# UPDATED: load_data now loads Conformer features instead of Wav2Vec2
#######################################################################################

def load_data(_feat_folder, _mono, _fold=None):
    feat_file_fold = os.path.join(_feat_folder, 'conformer_{}_fold{}.npz'.format('mon' if _mono else 'bin', _fold))
...
def get_conformer_model(data_in, data_out, _cnn_nb_filt, _cnn_pool_size, _rnn_nb, _fc_nb):
...
feat_folder = './feat_conformer/'   # folder containing Conformer embeddings
...
for fold in [1, 2, 3, 4]:
    ...
    # Build model
    model = get_conformer_model(X, Y, cnn_nb_filt, cnn_pool_size, rnn_nb, fc_nb)
